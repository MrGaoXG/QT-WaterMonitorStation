...
一些设备在Linux系统下回首先被识别为U盘并进行挂载，这个时候如果要使用设备，需要进行模式切换才能让系统把他识别成对应的设备

```
//usb-modeswitch 是 Linux 下专门用于切换 USB 设备模式的工具（很多 USB 设备初始会被识别为 “存储设备”，需切换后才能作为网卡 / 调制解调器使用）。
//-y 表示自动确认安装，无需手动输入 yes。
sudo apt-get install usb-modeswitch -y//安装USB功能切换工具
//向指定 USB 设备发送 “模式切换指令”，将其从存储模式切换为功能模式。
sudo usb_modeswitch -v 8087 -p 1024 -M '5553424312345678000000000000061b000000020000000000000000000000'
```
